 ...
 */
export function createLogger(namespace, metadata = {}) {
...
export function createServer() {
  ...
  if (process.env['SSL_CERT_PATH'] && process.env['SSL_KEY_PATH']) {
    try {
      ...
      console.warn('Failed to load SSL cert/key, falling back to HTTP:', /** @type {Error} */ (e).message);
  ...
  app.use((req, res, next) => {
    ...
    // @ts-ignore - Adding custom log property
    req.log = {
  ...
  // Error handling middleware (err is typed as 'any' per Express.ErrorRequestHandler signature)
  app.use((
    /** @type {any} */ err,
  ...
  function getRoom(id) {
    ...
    return /** @type {Set<import('ws').WebSocket>} */ (rooms.get(id));
  }

  /**
   * @param {string} roomId
   * @param {import('ws').WebSocket} fromWs
   * @param {any} payload
   */
  ...
  server.on('upgrade', (request, socket, head) => {
    const { pathname } = new URL(/** @type {string} */ (request.url), 'http://localhost');
    if (pathname === '/ws') {
